...
function submitPrompt(session, entry) {
  ...
  // writePromptNow left inFlight=false on failure; if the process is still
  // alive (write failed for a transient reason) the session would sit idle
  // without a reaper because the reuse path disarmed the timer before calling
  // us. Re-arm so the idle deadline still fires.
  if (isSessionProcessAlive(session) && !session.inFlight) armIdleTimer(session);
}
...
function drainQueue(session) {
  ...
  if (!writePromptNow(session, next)) {
    ...
    // inFlight stays false on failure. handleEvent disarmed the idle timer
    // when the prior result fired and expected drainQueue to either flip
    // inFlight=true (next prompt running) or leave the session truly idle —
    // in the latter case we must re-arm so the reaper still fires.
    if (isSessionProcessAlive(session) && !session.inFlight) armIdleTimer(session);
  }
}
